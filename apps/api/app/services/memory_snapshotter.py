...
Execution boundary (do not violate):
  ...
  * It reads the mock Memory provider through Engine -> Provider Registry
    resolution (the same surface the loop uses) — never a real database.
...
def take_snapshot(resident_id: str, run_id: str = "") -> Dict[str, Any]:
    ...
    """
    listing = route_provider_for_engine("memory_mock", {"op": "list", "resident_id": resident_id})
