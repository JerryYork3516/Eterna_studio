# Eterna Canvas — Schema Contract v0.1

> 状态：**合约 / PASS**（前身 SCHEMA_DRAFT v0.1 评审稿，已并入 GPT 6 项小修）。
> 后端 Pydantic 为唯一权威源；前端按本合约导出的 OpenAPI / JSON Schema 对接。
> 本合约一经确认，字段变更须走版本号递增（v0.1 → v0.2 ...）。

## 变更记录（相对草案）
- M1 第13层改 mixed lock 方案，锁定下沉到子节点。
- M2 LockLevel 新增 `mixed`（**仅** layer_container 可用）。
- M3 model 节点 data 改为 LiteLLM 兼容命名。
- M4 ChangeApproval.diff 约定最小结构 before/after/changed_fields。
- M5 删除 LayerContainerData.lock_level，唯一权威 = WorkflowNode.lock_level。
- M6 title 拆为 title_key + title_fallback。

---

## 0. 全局约定

- `schema_version`：所有核心顶层对象**必须**携带，字符串，形如 `"0.1.0"`。
...
## 8. Persona Builder —— 13 层默认结构  ← M1

| #  | title_key                   | title_fallback   | 默认 lock_level |
|----|-----------------------------|------------------|-----------------|
| 1  | layer.source_input          | 资料输入层       | editable        |
| 2  | layer.identity_core         | 身份核心层       | locked          |
| 3  | layer.legal_permission      | 授权权限层       | locked          |
| 4  | layer.safety_boundary       | 安全边界层       | locked          |
| 5  | layer.world_context         | 世界观环境层     | review_required |
| 6  | layer.personality           | 人格层           | review_required |
| 7  | layer.memory                | 记忆层           | review_required |
| 8  | layer.knowledge             | 知识层           | editable        |
| 9  | layer.relationship          | 关系层           | review_required |
| 10 | layer.behavior              | 行为层           | review_required |
| 11 | layer.capability_tools      | 能力工具层       | editable        |
| 12 | layer.multimodal            | 表现层           | review_required |
| 13 | layer.audit_export_deploy   | 审计输出部署层   | **mixed**       |
